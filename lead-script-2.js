...
(function() {
    ...
    function createLeadData(urlParams, shouldIncludeReferer = true, existingReferer = null) {
        ...
        // Always include referer for attribution backup
        if (shouldIncludeReferer) {
            parameters.http_referer = getHttpReferer();
    ...
    function determineLeadAction(urlParams, existingLead) {
        ...
        if (hasUrlParams && hasExistingLead) {
            ...
            if (!paramsMatch) {
                // UTM parameters changed - capture new referer
                return { action: 'replace', data: createLeadData(urlParams, true) };
            } else {
                // UTM parameters same - still capture current referer for attribution backup
                const updatedData = createLeadData(urlParams, true);
                return { action: 'replace', data: updatedData };
            }
        }
        
        if (!hasUrlParams && hasExistingLead) {
            // No UTM params but existing lead - preserve UTM data, capture current referer
            const existingUtmParams = {};
            for (const param of CONFIG.UTM_PARAMETERS) {
                existingUtmParams[param] = existingLead.parameters[param] || CONFIG.EMPTY_PARAM_VALUE;
            }
            const updatedData = createLeadData(existingUtmParams, true);
            return { action: 'replace', data: updatedData };
        }
